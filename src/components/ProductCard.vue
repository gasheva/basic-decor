...
<base-card>
   <template #default>
       <div class="d-flex align-items-center mb-1">
           <input :id="checkboxId" class="form-check-input mt-0" type="checkbox" :checked="product.selected"
                  aria-label="Select all" @click.capture="toggleSelection">
           <label :for="checkboxId" class="ms-2" @click.prevent>Выбрать</label>
           <div class="d-flex align-items-center justify-content-center ms-auto bg-primary" :class="$style.favourite"><i class="bi bi-heart text-white"></i></div>
       </div>
    ...
    <div class="mt-auto"><span class="fs-3 fw-bold">{{ formattedPrice }}</span><span class="fw-bold">&nbsp;руб.</span></div>
   </template>
</base-card>
</template>
