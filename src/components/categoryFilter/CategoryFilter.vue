...
const onSelectAll = (event: Event) => {
  ...
  emit('toggleAll', selectAll.value)
}
</script>
...
<template>
  <div class="d-flex align-items-center border w-25">
    ...
    <div class="dropdown w-100" :class="$style.dropdown">
      <button
        class="btn bg-white dropdown-toggle w-100 d-flex justify-content-between align-items-center"
        :class="$style.button"
        type="button"
      ...
      <ul class="dropdown-menu w-100" aria-labelledby="dropdownMenuButton1">
        <li v-for="category in categories" :key="category.id">
          <a
            ...
            :class="[$style.dropdownItem, { 'text-primary': selectedCategory.id === category.id }]"
          ...
          <ul class="ps-3">
            <li v-for="child in category.children" :key="child.id" class="list-unstyled">
              <a
                class="dropdown-item text-wrap"
                :class="[$style.dropdownItem, { 'text-primary': selectedCategory.id === child.id }]"
                @click.prevent="onSelect(child)"
                >{{ `${child.name} (${child.productsCount})` }}</a
              >
            </li>
          </ul>
        </li>
